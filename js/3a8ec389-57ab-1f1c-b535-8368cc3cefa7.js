var timeout,
  ...
  firstTimeLoaded = 0,
  fontSize = 100,
  path = "/apps-data/reading-for-the-real-world-1/",
...
$(function () {
  ...
  ($.ajax({
    ...
    $(window).resize(function () {
      ...
      (resizeLastSession(), resizeExerciseStory(), resizePageContentImg());
    }),
    ...
    $("#section-section").click(function () {
      ($("#category-section").find(".container").html(""),
        ...
        $("#bottom-section")
          .children(".container")
          .html(
            '<div class="app-name">Reading for the Real World 1</div ><div class="app-slogan">4th Edition</div>',
          ),
...
function getSectionItem(e) {
  if (null != localStorage[path.split("/")[2] + ":" + e]) {
    var t = JSON.parse(localStorage.getItem(lsetting));
    ...
    var r = JSON.parse(
        localStorage.getItem(path.split("/")[2] + ":" + e),
      ).wordlist,
      o = JSON.parse(
        localStorage.getItem(path.split("/")[2] + ":" + e),
      ).reading;
    if (null != o) {
      ...
      var l = JSON.parse(localStorage.getItem(path.split("/")[2] + ":" + e)).en,
        s = "";
      ("" != l && (s += '<div class="wordlist-rotate">' + l + "</div>"),
        (s += '<div class="en-story">' + o + "</div>"),
        (s +=
          ...
          (0 < $("#idiom-tip-popup").length && $("#idiom-tip-popup").remove(),
            $("body").append(
              '<div id="idiom-tip-popup"><div id="close-idiom-tip-popup"><img src="icons/close-window-50.png" /></div><div id="idiom-tip-popup-cover">' +
                $(".vocabulary ul li").eq($(this).attr("tabindex")).html() +
                "</div></div>",
            ),
                        ...
                        ($(this).attr("start") != null &&
                          ...
                          $(this)
                            ...
                            .each(function () {
                              ...
                              (d.toLocaleLowerCase() == s.toLocaleLowerCase()
                                ? $("#exercise-answer-key table").append(
                    ...
                    $(this)
                      ...
                      .each(function (e) {
                        ...
                        for (
                          ...
                          if ($.trim(r[d]) == n.toLocaleLowerCase()) {
                            o = !0;
                            break;
                          }
                        !0 == o && "" != n.toLocaleLowerCase()
                          ? $("#exercise-answer-key table").append(
                              '<tr style="background:#1EA362; color:white"><td>' +
                                (e + 1) +
                                "</td><td>" +
        ...
        $(".speaker-louder").each(function (e) {
          ($(this).attr("idx", e), (d += $(this).attr("source") + ","));
        }),
        "" != d && (d = d.substring(0, d.length - 1)),
        $("#item0").attr("soundarr", d),
        $("#item0").attr("si", 0));
    }
...
function playSound(t, e) {
  ...
  (0 < n.length
    ? ($("#bottom-section")
        .children(".container")
        .html(
          '<ul id="nav-story"><li style="display:none" source="' +
            e +
            '"><img class="audio-play" status="0" src="icons/play.png" /><span style="margin-left:5px; position:relative; top:-23px"></span></li><li style="width: calc(33% - 10px); display: none"></li><li style="color:yellow; width: 100%;">' +
            r +
            "</li></ul>",
        ),
...
function homeScroll() {
  ($(window).scroll(function () {
    ...
    (0 < $(".story-block-cover").length &&
      ((r = $(".section-rotate").eq(0).position().top),
      (o = $(".section-rotate").eq(1).position().top),
      (d = $(".section-rotate").eq(2).position().top),
      (s = $(".section-rotate").eq(3).position().top),
      (top5 = $(".section-rotate").eq(4).position().top)),
      n >= r - parseInt($(window).height()) + 150 &&
        $("#bottom-section")
          .find("#nav-story")
          .children("li")
          .eq(1)
          .html("Reading Comprehension"),
      n >= o - parseInt($(window).height()) + 150 &&
        $("#bottom-section")
          .find("#nav-story")
          .children("li")
          .eq(1)
      ...
      n >= d - parseInt($(window).height()) + 150 &&
        $("#bottom-section")
          ...
          .children("li")
          .eq(1)
          .html("Vocabulary Practice"),
      n >= s - parseInt($(window).height()) + 150 &&
        $("#bottom-section")
          .find("#nav-story")
          .children("li")
          .eq(1)
          .html("Supplemental Reading"),
      n >= top5 - parseInt($(window).height()) + 150 &&
        $("#bottom-section")
          .find("#nav-story")
          .children("li")
          .eq(1)
          .html("Vocabulary Preview"));
  }),
