  ...
  (loadLastSession(),
    ...
    $("#section-section").click(function () {
      (0 < $("#idiom-tip-popup").length && $("#idiom-tip-popup").remove(),
        clearInterval(adsFirstTimeLoadedInterval),
        ...
        $("#bottom-section")
          .children(".container")
          .html(
            '<div class="app-name">Most Common Around Town Idioms in English</div><div class="app-slogan">Hundreds of idiomatic expressions to give you an edge in English!</div>',
          ),
  ...
  if (null != sessionStorage[e]) {
    ...
    if (null != h) {
      ...
      for (n = 0; n < h.length; n++) {
        ...
        ((a = h[n].image), (l = h[n].sound));
        if ("story" == u) {
          var w = {
            title: (k = JSON.parse(localStorage.getItem("lastestSession")))
              .title,
            image: $("#home-section")
              .children("ul")
              .children("li")
          ...
          localStorage.setItem("lastestSession", JSON.stringify(w));
          var g = "word-image",
            y = "";
          (-1 < s.indexOf("Pronunciation Pointer") ||
            -1 < s.indexOf("Fun Fact!") ||
            -1 < s.indexOf("Did you spot it?")) &&
            ((g = "word-image-small"), (y = " pronunciation-pointer"));
          c = '<div class="clear-both' + y + '">';
          ("" != a &&
            ((c +=
              '<div class="' +
              g +
        ...
        n == h.length - 1 &&
          ...
          ($("#item" + (i.length + 1))
            .find(".en-story")
            .append(
              '<div class="reading-section-red-answer-key">Answer Key</div>',
            ),
    ...
    var x = JSON.parse(sessionStorage.getItem(e)).idioms;
    if (null != x || null != x) {
      ...
      for (n = 0; n < x.length; n++) {
        ...
        ((c += '<div class="wordlist-cover">'),
          (c += '<div><span class="word">' + s + ":</span> " + o + "</div>"),
          (c +=
            '<div  class="exam"><span class="arrow">→</span> ' + d + "</div>"),
          (c += "</div>"),
          (c += "</li>"),
    ...
    if (null != f)
      if (
        ...
        for (n = 0; n < 1; n++) {
          ...
          var b = f[n].story;
          if (0 == n) {
            var k = JSON.parse(localStorage.getItem("lastestSession")),
              _ = 0;
            (null == i && null == i) || (_ = i.length + 1);
            w = {
              title: k.title,
              image: $("#home-section")
                .children("ul")
        ...
        for (n = 0; n < f.length; n++) {
          ...
          if (0 == n) {
            ((k = JSON.parse(localStorage.getItem("lastestSession"))), (_ = 0));
            null != i || null != i
              ? (_ = i.length + 1)
              : $("#category-section")
                  .children(".section")
                  .children(".container")
                  .append(
                    '<li exercise="1" id="item' + _ + '"><div></div></div>',
                  );
            w = {
              title: k.title,
              image: $("#home-section")
...
function playSound(i, e) {
  ...
  if (1 == isExercise()) {
    ...
    if (
      ($("#bottom-section").css({ display: "block" }),
      0 < (r = $("#tbl-last-session" + (parseInt(n.index) + 1))).length)
    ) {
      n = JSON.parse(localStorage.getItem("lastestSession"));
      var s = r.parent().children("div").attr("en");
      0 < n.sindex
        ? $("#bottom-section")
            .children(".container")
            .html(
    ...
    $("#bottom-section").css({ display: "block" });
    s = "";
    if (0 < (r = $("#tbl-last-session" + (parseInt(n.index) + 1))).length)
      s = r.parent().children("div").attr("en");
    (0 == autoPlay
      ? $("#bottom-section")
          .children(".container")
          .html(
